...
export default function ChronoScriptEditor() {
  ...
  async function handleScan() {
    ...
    try {
      ...
      // Log to console
      console.log("ChronoScript Output:\n", result);
    } catch (err) {
  ...
  function extractSection(
    output: string,
    section: "LEXICAL" | "SYNTAX" | "SEMANTIC"
  ) {
    if (!output) return "No output";

    const start = output.indexOf(`--- PHASE: ${section}`);
    if (start === -1) return "No output";
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 shadow-xl">
                ...
                <pre className="font-mono text-xs text-slate-200 whitespace-pre-wrap">
                  {extractSection(output, "SEMANTIC")}
                </pre>
              </div>
            </div>
          </div>
...
schedule startClock() {
    ...
    tickout "Enter your age in seconds: ";
    tickin age;
    
    when (age >= 5) {
        tickout "your age is bigger";
    }
    otherwise {
        tickout "your name is small" + 5 + "HI";
    }
    
    tickout "Hello, " + name + "! Age seconds = " + age;
...
`)
      ...
      <footer className="border-t border-slate-800 bg-slate-950/50 mt-12">
        <div className="max-w-7xl mx-auto px-6 py-6 text-center text-sm text-slate-500">
          <p>3AM Code Scanner v1.0 • Built for developers</p>
        </div>
      </footer>
    </div>
  );
